...
async def lifespan(app: FastAPI):
    ...
    if s.payments_monitor_enabled and s.database_url.strip():
        ...
        logger.info(
            "Monitor de pagos activo (cada %ss, IMAP FROM OR=%s, TEXT OR=%s, droguerías id=%s / secundaria=%s si match %r)",
            s.payments_monitor_interval_sec,
            s.bancol_notifications_from,
...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        description="API para verificación de pagos vía correo (IMAP), QR, WebSocket, ventas, estadísticas y facturas.",
        version="0.5.0",
        lifespan=lifespan,
    )
